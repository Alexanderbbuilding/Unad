...
require_once("../conexion/User.php")   ;
	...
	if( !empty( $_POST )){
		try {
			...
			$data = $user_obj->addcargo( $_POST );
				if($data['status']){
				if($data['operation'] == 'update')$_SESSION['success'] = "Cargo Actualizado";
				else $_SESSION['success'] = "Cargo Agregado";
				header('Location: cargo.php');exit;
			}else{
				$_SESSION['success'] = "El Cargo No Pudo Ser Actualizado";
				header('Location: cargo.php');exit;
			}
		...
		try {
			$user = new Cl_User();
			$results = $user->getcargo( $_GET['uuid'] );
		} catch (Exception $e) {
			$_SESSION['error'] = $e->getMessage();
		}
	}
	...
	$id = isset($results['idCargo']) ? $results['idCargo']: '';
	if(!empty($id)){
		$head_txt = 'Cargo a editar';
		$txt = 'Actualizar Cargo';
	}else{
		$head_txt = 'Agregar Cargo';
		$txt = 'Agregar';
	} 
?>
	...
	<div class="col-xs-9 col-sm-10 col-md-10 col-lg-10 col-xs-offset-3 col-sm-offset-2 col-md-offset-2 col-lg-offset-2 main">
		...
		<form action="<?php echo $_SERVER['PHP_SELF']; ?>" id="product-form" method="post" class="form-horizontal myaccount" role="form">
			<div class="load-animate">
				...
				<input type="hidden" id="idCargo" name="idCargo" value="<?php echo $id; ?>">
